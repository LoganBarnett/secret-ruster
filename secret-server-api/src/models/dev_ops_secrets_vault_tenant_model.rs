/*
 * Secret Server Rest API
 *
 * REST API documentation for Secret Server. This document describes how to use the REST API. All requests require an authentication token; please see the <a href=\"../OAuth/\">authentication document</a> for more information. The <a href=\"swagger.json\">Swagger specification</a> for this API is also available.
 *
 * The version of the OpenAPI document: 11.7.61
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// DevOpsSecretsVaultTenantModel : DevOps Secrets Vault Tenant
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct DevOpsSecretsVaultTenantModel {
    /// Active
    #[serde(rename = "active", skip_serializing_if = "Option::is_none")]
    pub active: Option<bool>,
    /// Date Added
    #[serde(rename = "dateAdded", skip_serializing_if = "Option::is_none")]
    pub date_added: Option<String>,
    /// DSV Tenant Id
    #[serde(rename = "dsvTenantId", skip_serializing_if = "Option::is_none")]
    pub dsv_tenant_id: Option<i32>,
    /// Last Synchronized
    #[serde(rename = "lastSynced", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub last_synced: Option<Option<String>>,
    /// Secret Id
    #[serde(rename = "secretId", skip_serializing_if = "Option::is_none")]
    pub secret_id: Option<i32>,
    /// Secret Name
    #[serde(rename = "secretName", skip_serializing_if = "Option::is_none")]
    pub secret_name: Option<String>,
    /// Synchronization Interval
    #[serde(rename = "syncInterval", skip_serializing_if = "Option::is_none")]
    pub sync_interval: Option<i32>,
    /// Tenant Name
    #[serde(rename = "tenantName", skip_serializing_if = "Option::is_none")]
    pub tenant_name: Option<String>,
}

impl DevOpsSecretsVaultTenantModel {
    /// DevOps Secrets Vault Tenant
    pub fn new() -> DevOpsSecretsVaultTenantModel {
        DevOpsSecretsVaultTenantModel {
            active: None,
            date_added: None,
            dsv_tenant_id: None,
            last_synced: None,
            secret_id: None,
            secret_name: None,
            sync_interval: None,
            tenant_name: None,
        }
    }
}

