/*
 * Secret Server Rest API
 *
 * REST API documentation for Secret Server. This document describes how to use the REST API. All requests require an authentication token; please see the <a href=\"../OAuth/\">authentication document</a> for more information. The <a href=\"swagger.json\">Swagger specification</a> for this API is also available.
 *
 * The version of the OpenAPI document: 11.7.61
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// FolderSimpleModel : Describes simple properties of a secret folder.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct FolderSimpleModel {
    /// The ID for the folder.
    #[serde(rename = "id", skip_serializing_if = "Option::is_none")]
    pub id: Option<i32>,
    /// The dispay name for the folder.
    #[serde(rename = "name", skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// The full path for the folder.
    #[serde(rename = "path", skip_serializing_if = "Option::is_none")]
    pub path: Option<String>,
    /// Whether the folder is a restricted system folder.
    #[serde(rename = "restricted", skip_serializing_if = "Option::is_none")]
    pub restricted: Option<bool>,
}

impl FolderSimpleModel {
    /// Describes simple properties of a secret folder.
    pub fn new() -> FolderSimpleModel {
        FolderSimpleModel {
            id: None,
            name: None,
            path: None,
            restricted: None,
        }
    }
}

