/*
 * Secret Server Rest API
 *
 * REST API documentation for Secret Server. This document describes how to use the REST API. All requests require an authentication token; please see the <a href=\"../OAuth/\">authentication document</a> for more information. The <a href=\"swagger.json\">Swagger specification</a> for this API is also available.
 *
 * The version of the OpenAPI document: 11.7.61
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// TestRemoteScriptSshResponseModel : Output from a ssh script test
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct TestRemoteScriptSshResponseModel {
    /// Debug output from test
    #[serde(rename = "debugOutput", skip_serializing_if = "Option::is_none")]
    pub debug_output: Option<Vec<String>>,
    /// Error message from test
    #[serde(rename = "errorMessage", skip_serializing_if = "Option::is_none")]
    pub error_message: Option<String>,
    /// Error output from test
    #[serde(rename = "errorOutput", skip_serializing_if = "Option::is_none")]
    pub error_output: Option<Vec<String>>,
    /// Log Messages from test
    #[serde(rename = "logMessages", skip_serializing_if = "Option::is_none")]
    pub log_messages: Option<Vec<String>>,
}

impl TestRemoteScriptSshResponseModel {
    /// Output from a ssh script test
    pub fn new() -> TestRemoteScriptSshResponseModel {
        TestRemoteScriptSshResponseModel {
            debug_output: None,
            error_message: None,
            error_output: None,
            log_messages: None,
        }
    }
}

