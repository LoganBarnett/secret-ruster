/*
 * Secret Server Rest API
 *
 * REST API documentation for Secret Server. This document describes how to use the REST API. All requests require an authentication token; please see the <a href=\"../OAuth/\">authentication document</a> for more information. The <a href=\"swagger.json\">Swagger specification</a> for this API is also available.
 *
 * The version of the OpenAPI document: 11.7.61
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// SecretTemplateExportModel : Secret Template Export Model
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct SecretTemplateExportModel {
    /// ExportFileText
    #[serde(rename = "exportFileText", skip_serializing_if = "Option::is_none")]
    pub export_file_text: Option<String>,
    /// FileName
    #[serde(rename = "fileName", skip_serializing_if = "Option::is_none")]
    pub file_name: Option<String>,
}

impl SecretTemplateExportModel {
    /// Secret Template Export Model
    pub fn new() -> SecretTemplateExportModel {
        SecretTemplateExportModel {
            export_file_text: None,
            file_name: None,
        }
    }
}

