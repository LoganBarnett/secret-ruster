/*
 * Secret Server Rest API
 *
 * REST API documentation for Secret Server. This document describes how to use the REST API. All requests require an authentication token; please see the <a href=\"../OAuth/\">authentication document</a> for more information. The <a href=\"swagger.json\">Swagger specification</a> for this API is also available.
 *
 * The version of the OpenAPI document: 11.7.61
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// SshKeySizeType : SSH Key Size Type
/// SSH Key Size Type
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum SshKeySizeType {
    #[serde(rename = "Key1024")]
    Key1024,
    #[serde(rename = "Key2048")]
    Key2048,
    #[serde(rename = "Key4096")]
    Key4096,

}

impl std::fmt::Display for SshKeySizeType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::Key1024 => write!(f, "Key1024"),
            Self::Key2048 => write!(f, "Key2048"),
            Self::Key4096 => write!(f, "Key4096"),
        }
    }
}

impl Default for SshKeySizeType {
    fn default() -> SshKeySizeType {
        Self::Key1024
    }
}

