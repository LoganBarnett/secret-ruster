...
/// struct for typed errors of method [`role_audit_service_get_role_audits`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RoleAuditServiceGetRoleAuditsError {
...
pub async fn role_audit_service_get_role_audits(configuration: &configuration::Configuration, is_exporting: Option<bool>, filter_audit_type: Option<&str>, skip: Option<i32>, sort_by_left_square_bracket_0_right_square_bracket__direction: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__name: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__priority: Option<i32>, take: Option<i32>) -> Result<models::PagingOfRoleAuditSummary, Error<RoleAuditServiceGetRoleAuditsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_is_exporting = is_exporting;
    let p_query_filter_audit_type = filter_audit_type;
    ...
    let uri_str = format!("{}/v1/role-audits", configuration.base_path);
    ...
    if let Some(ref param_value) = p_query_filter_audit_type {
        req_builder = req_builder.query(&[("filter.auditType", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PagingOfRoleAuditSummary`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PagingOfRoleAuditSummary`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<RoleAuditServiceGetRoleAuditsError> = serde_json::from_str(&content).ok();
