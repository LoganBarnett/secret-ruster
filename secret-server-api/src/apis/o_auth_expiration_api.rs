...
/// struct for typed errors of method [`o_auth_expiration_service_expire_session`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OAuthExpirationServiceExpireSessionError {
...
pub async fn o_auth_expiration_service_expire_session(configuration: &configuration::Configuration, ) -> Result<bool, Error<OAuthExpirationServiceExpireSessionError>> {

    let uri_str = format!("{}/v1/oauth-expiration", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<OAuthExpirationServiceExpireSessionError> = serde_json::from_str(&content).ok();
