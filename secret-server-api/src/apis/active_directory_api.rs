...
/// struct for typed errors of method [`active_directory_service_synchronize`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ActiveDirectoryServiceSynchronizeError {
...
pub async fn active_directory_service_synchronize(configuration: &configuration::Configuration, ) -> Result<bool, Error<ActiveDirectoryServiceSynchronizeError>> {

    let uri_str = format!("{}/v1/active-directory/synchronize", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ActiveDirectoryServiceSynchronizeError> = serde_json::from_str(&content).ok();
