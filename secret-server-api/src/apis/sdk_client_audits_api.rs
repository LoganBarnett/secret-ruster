...
/// struct for typed errors of method [`sdk_client_audits_service_search_client_audit`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SdkClientAuditsServiceSearchClientAuditError {
...
pub async fn sdk_client_audits_service_search_client_audit(configuration: &configuration::Configuration, filter_operator: Option<&str>, filter_search_text: Option<&str>, filter_user_id: Option<i32>, skip: Option<i32>, sort_by_left_square_bracket_0_right_square_bracket__direction: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__name: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__priority: Option<i32>, take: Option<i32>) -> Result<models::PagingOfSdkClientAuditSummaryAndSdkClientAuditFilter, Error<SdkClientAuditsServiceSearchClientAuditError>> {
    ...
    let uri_str = format!("{}/v1/sdk-client-audits", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PagingOfSdkClientAuditSummaryAndSdkClientAuditFilter`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PagingOfSdkClientAuditSummaryAndSdkClientAuditFilter`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<SdkClientAuditsServiceSearchClientAuditError> = serde_json::from_str(&content).ok();
