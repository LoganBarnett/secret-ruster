...
/// struct for typed errors of method [`hsm_configuration_service_get_hsm_configuration_audit`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HsmConfigurationServiceGetHsmConfigurationAuditError {
...
pub async fn hsm_configuration_service_get_hsm_configuration_audit(configuration: &configuration::Configuration, is_exporting: Option<bool>, filter_search_term: Option<&str>, skip: Option<i32>, sort_by_left_square_bracket_0_right_square_bracket__direction: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__name: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__priority: Option<i32>, take: Option<i32>) -> Result<models::PagingOfHsmConfigurationAuditViewModel, Error<HsmConfigurationServiceGetHsmConfigurationAuditError>> {
    ...
    let uri_str = format!("{}/v1/hsm-configuration/audit", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PagingOfHsmConfigurationAuditViewModel`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PagingOfHsmConfigurationAuditViewModel`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HsmConfigurationServiceGetHsmConfigurationAuditError> = serde_json::from_str(&content).ok();
