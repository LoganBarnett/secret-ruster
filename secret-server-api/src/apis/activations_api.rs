...
/// struct for typed errors of method [`activations_service_activate`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ActivationsServiceActivateError {
...
pub async fn activations_service_activate(configuration: &configuration::Configuration, args: Option<models::OnlineActivationArgs>) -> Result<bool, Error<ActivationsServiceActivateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_args = args;

    let uri_str = format!("{}/v1/activations", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ActivationsServiceActivateError> = serde_json::from_str(&content).ok();
