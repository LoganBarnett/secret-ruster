...
/// struct for typed errors of method [`event_pipeline_audit_service_get_all_pipeline_and_policy_audits`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum EventPipelineAuditServiceGetAllPipelineAndPolicyAuditsError {
...
pub async fn event_pipeline_audit_service_get_all_pipeline_and_policy_audits(configuration: &configuration::Configuration, is_exporting: Option<bool>, filter_event_pipeline_policy_id: Option<i32>, skip: Option<i32>, sort_by_left_square_bracket_0_right_square_bracket__direction: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__name: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__priority: Option<i32>, take: Option<i32>) -> Result<models::PagingOfEventPipelinePolicyAuditSummary, Error<EventPipelineAuditServiceGetAllPipelineAndPolicyAuditsError>> {
    ...
    let uri_str = format!("{}/v1/event-pipeline-audit", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PagingOfEventPipelinePolicyAuditSummary`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PagingOfEventPipelinePolicyAuditSummary`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<EventPipelineAuditServiceGetAllPipelineAndPolicyAuditsError> = serde_json::from_str(&content).ok();
