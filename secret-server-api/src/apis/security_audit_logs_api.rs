...
/// struct for typed errors of method [`security_audit_logs_service_search_logs`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SecurityAuditLogsServiceSearchLogsError {
...
pub async fn security_audit_logs_service_search_logs(configuration: &configuration::Configuration, filter_is_exporting: Option<bool>, filter_search_text: Option<&str>, skip: Option<i32>, sort_by_left_square_bracket_0_right_square_bracket__direction: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__name: Option<&str>, sort_by_left_square_bracket_0_right_square_bracket__priority: Option<i32>, take: Option<i32>) -> Result<models::PagingOfSecurityAuditLogSummaryAndSecurityAuditLogFilter, Error<SecurityAuditLogsServiceSearchLogsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_filter_is_exporting = filter_is_exporting;
    ...
    let uri_str = format!("{}/v1/security-audit-logs", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    if let Some(ref param_value) = p_query_filter_is_exporting {
        req_builder = req_builder.query(&[("filter.isExporting", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PagingOfSecurityAuditLogSummaryAndSecurityAuditLogFilter`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PagingOfSecurityAuditLogSummaryAndSecurityAuditLogFilter`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<SecurityAuditLogsServiceSearchLogsError> = serde_json::from_str(&content).ok();
